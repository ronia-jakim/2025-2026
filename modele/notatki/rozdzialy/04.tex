...
\begin{proof}
  \begin{enumerate}
    \item Ma model pierwszy, bo jest $\aleph_0$-stabilna. Twierdze, że jest nim $(\Z, S)$
      \begin{description}
        \item[pierwszość=atomowość] bierę dowolny $(a_1,...,a_n)\in\Z^n$, jego typ jest izolowany przez $\phi(x_1,...,x_n)="x_1=S^{x_1-x_2}(x_2)\;\land x_1=S^{x_1-x_3}(x_3)\;\land\;..."$
    ...
    \item Ma model pierwszy, gdyż iż i ponieważ jest $\aleph_0$-stabilne. Zapewne to jest przestrzeń wektorowa o nieskończonej, przeliczalnej bazie - inaczej: ciągi elementów z $F$ o skończenie wielu niezerowych elementach.
      ...
      \end{description}
    \item Ma model pierwszy jeszcze raz, bo jest $\aleph_0$-stabilne. Zapewne znowu to będzie nieskończenie wymiarowa przestrzeń liniowa i argumenty będą bardzo podobne jak wcześniej.
  \end{enumerate}
\end{proof}
...
\begin{proof}
  \begin{enumerate}
    \item Niech $M$ będzie nieminimalnym modelem pierwszym $T$. Fakt z wykładu:
      ...
      \end{fact}
      Przeliczalność to jakiś turbo szybki dowód, który mówi że skoro pracujemy nad przeliczalnym językiem to mamy jakiś przeliczlany model.

      Ponieważ $M$ nie jest minimalne, to zawiera $N\prec M$, $N\neq M$. Ale $M$ jest pierwsze, więc istnieje $f:M\to N\subseteq M$ elementarne zanurzenie $M$ w samego siebie takie, że $f(M)\subsetneq M$. Ja chcę jednak żyć w dużym modelu monstrum gdzie będę potrafiła to $f$ odwrócić. Jakiś automorfizm czy coś, bo orbity działania grupy automorfizmów powinny być w bijekcji z typami. 

      Generalnie chcę przekonać się, że $M\supseteq f^{-1}(M)$, żeby móc stworzyć ciąg $M_0=M\prec M_1=f^{-1}(M)\prec....$ i wziąć jego (ko)granicę? Injective limit z pierwszej listy.
      
      Powinnam uzasadnić, że $\varinjlim M_i$ które dostaję jest atomowe. Każde $M_i$ powinno być atomowe, bo jest obrazem przez elegancką funkcję modelu atomowego i z tego też powodu granica powinna być atomowa.
  
      Dlaczego to co dostaję jest nieprzeliczalne? Bo mogę tę konstrukcję robić nieprzeliczalną ilość kroków, za każdym razem zwiększając mój model o co najmniej jeden element. 
  \end{enumerate}
\end{proof}
