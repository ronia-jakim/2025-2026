...
Niech teraz $K=\C$. Na $\C[G]$ zadajemy inwolucję wzorem
$$f^*(g)=\overline{f(g^{-1})}.$$
...
Nie jest jednak tak źle - mimo że prawdziwy element neutralny w $L^1(G)$ nie istnieje to możemy wyprodukować coś bardzo blisko bycia jedynką na splot.

\begin{example}
  Rozważmy grupę $S^1$, czyli liczby zespolone o module $1$ z mnożeniem. Dla $0\leq r< 1$ zdefiniujmy odwzorowanie
  $$P_r(\theta)=\sum_{n\in\Z}r^{|n|}|e^{in\theta}=\frac{1-r^2}{|1-re^{i\theta}|^2}\geq0.$$
 Weźmy teraz $f\in L^1(S^1)$, wówczas zachodzi
 $$\|P_r\ast f-f\|_{L^1}\xrightarrow{r\to 1} 0,$$
 czyli $P_r\ast f$ zgadza się z $f$ prawie wszędzie dla dowolnego $f\in L^1(S^1)$. Funkcję $P_r$ tak zdefiniowaną nazywamy \buff{jądrem Poissona}.
\end{example}
...
\begin{theorem}{}{alg grupowa to suma algebr macierzowych}
...
\begin{proof}
  ...
  \begin{description}
    ...
    \item[injektywność:] gdyby istniało $f\in \C[G]$ takie, że $f\mapsto 0$, to dla każdej nieprzywiedlniej $\rho_i$ musi zachodzić $\rho_i(f)=0$. W szczególności dla lewej reprezentacji regularnej, która jest wierna (injektywna), musi zachodzić $\lambda(f)=0$. 
  \end{description}
\end{proof}


\begin{theorem}{}{}
  Niech $G$ będzie grupą zwartą, a $\pi$ będzie jej unitarną, nieprzywiedlnią reprezentacją. Wówczas $\pi$ jest podreprezentacją lewej regularnej reprezentacji grupy $G$ z krotnością $\dim(\pi)$.
\end{theorem}

\begin{proof}
  Oznaczmy lewą regularną reprezentację $G$ przez $\lambda_G$. Wówczas jej charakter to
  $$\chi_{\lambda_G}(g)=\begin{cases}|G| & g=1\\ 0 & g\neq 1\end{cases}.$$
  % $\lambda_G$ działa na przestrzeni $L^2(G)$ przez lewe translacje.
  Z kolei na mocy twierdzenia \ref{alg grupowa to suma prosta algebr macierzowych} wiemy, że 
  $$|G|=\sum\dim(\pi)^2,$$
  {\color{red}wystarczy uczciwie policzyć $\langle \lambda_G, \pi\rangle$}
\end{proof}
